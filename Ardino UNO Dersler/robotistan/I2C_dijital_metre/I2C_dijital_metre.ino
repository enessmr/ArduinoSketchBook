#include <LiquidCrystal_I2C.h>                              //LCD kütüphanemizi başlatıyoruz.
...
LiquidCrystal_I2C lcd(0x27, 16, 2);              //LCD'nin pin bağlantılarını ayarlıyoruz.

void setup() {
  lcd.backlight();
  pinMode(trigPin, OUTPUT);                             //trig pinini OUTPUT olarak ayarlıyoruz.
  pinMode(echoPin,INPUT);                               //echo pinini INPUT olarak ayarlıyoruz.
  lcd.init();                                     //LCD ekranımızın en-boy oranını ayarlıyoruz.                               
}
void loop() {
  ...
  //uzaklik= sure*0.0290/2;                               //Ölçülen süre ile uzaklık hesabı yapıyoruz.

  lcd.clear();                                          //LCD'deki eski yazılar temizlenir.
  lcd.setCursor(0, 1);                                  //LCD'nin 1. satır 1. sütunundan yazmaya başlıyoruz.      
  lcd.print("Uzaklik:");                                
  lcd.setCursor(0, 0);                                  //LCD'nin 2. satır 1. sütunundan yazmaya başlıyoruz.
